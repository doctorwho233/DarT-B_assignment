...
MySQL 최신 버전에서는 파생 테이블과 JOIN으로 최적화되는 경우가 있다.

7. 주의사항

스칼라 서브쿼리는 반드시 하나의 값만 반환해야 한다.

NULL 처리: ANY, ALL, IN, NOT IN에서 NULL이 포함되면 예상치 못한 결과가 발생할 수 있다.

NOT IN과 <> ALL 은 동일하게 해석된다.
